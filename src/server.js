...
async function handleWebhookEvent(body) {
  ...
  // Clave de memoria por remitente: cada persona que escribe por DM tiene su
  // propio hilo de conversacion, independiente del resto.
...
async function handleCommentEvent(value) {
  ...
  // Clave de memoria por publicacion + comentarista: si la misma persona
  // comenta varias veces en la misma publicacion, se le sigue el hilo.
  const conversationKey = `comment:${value.media?.id || "sin-media"}:${fromId || commentId}`;
  ...
  const audience = detectAudience(text);
      const history = getHistory(conversationKey);
      const reply = await generateReply({ text, audience, channel: "comment", history });
