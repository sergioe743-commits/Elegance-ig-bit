...
function initSchema() {
  ...
  // Google Sheets is authoritative for confirmed cash received. If a later AI
  // conversation analysis tries to replace a confirmed sale, restore the
  // spreadsheet-backed values immediately.
  db.exec(`
...
function isTreatmentRow(treatment, amount) {
  ...
  // Administrative/ancillary rows are revenue but not a treatment conversion.
  if (/^(pv|primera visita)\b/.test(text)) return false;
  if (/\breserva\b/.test(text)) return false;
  if (/\bmasaj/.test(text)) return false;
  if (/\bretoque\b/.test(text)) return false;
  if (/\bsin coste\b/.test(text)) return false;
  return amount > 0;
}
...
function findHeaderRow(rows) {
  for (let i = 0; i < Math.min(rows.length, 30); i += 1) {
    ...
    const hasDate = normalized.some((v) => v === "fecha");
    const hasName = normalized.some((v) => v === "nombre");
    const hasTreatment = normalized.some((v) => v === "tratamiento");
    const hasAmount = normalized.some((v) => v === "abonado");
    if (hasDate && hasName && hasTreatment && hasAmount) return i;
  }
  return -1;
}
  ...
  const tx = db.transaction(() => {
    for (let i = headerIndex + 1; i < rows.length; i += 1) {
      ...
      upsertSale.run({
        ...
        wa_id: waId,
        is_treatment: isTreatmentRow(treatment, amount) ? 1 : 0,
        sync_batch: batch,
        synced_at: now,
      });
      imported += 1;
    ...
    // Remove stale rows from this monthly tab only after a successful complete read.
...
function leadStartDate(firstSeenAt) {
  ...
  // One-day tolerance for timezone / form-vs-WhatsApp ordering.
...
function applyAttribution() {
  ...
  const salesForLead = db.prepare(`
    SELECT amount, is_treatment, sale_date
    FROM treatment_sales
    WHERE wa_id = ?
      AND amount > 0
      AND (? IS NULL OR sale_date IS NULL OR sale_date >= ?)
    ORDER BY sale_date ASC, row_number ASC
  `);
  const updateLead = db.prepare(`
    UPDATE leads
    SET sales_revenue = @sales_revenue,
        sales_treatment_revenue = @sales_treatment_revenue,
        sales_count = @sales_count,
        sales_last_at = @sales_last_at,
        revenue = CASE WHEN @sales_revenue > 0 THEN @sales_revenue ELSE revenue END,
        treatment_status = CASE WHEN @sales_treatment_revenue > 0 THEN 'completed' ELSE treatment_status END,
        funnel_stage = CASE WHEN @sales_treatment_revenue > 0 THEN 'treated' ELSE funnel_stage END,
  ...
  const tx = db.transaction(() => {
    for (const lead of leads) {
      ...
      const treatmentRevenue = sales
        .filter((s) => Number(s.is_treatment) === 1)
        .reduce((sum, s) => sum + Number(s.amount || 0), 0);
      const lastSaleAt = sales.map((s) => s.sale_date).filter(Boolean).sort().pop() || null;
      updateLead.run({
        wa_id: lead.wa_id,
        sales_revenue: revenue,
        sales_treatment_revenue: treatmentRevenue || null,
        sales_count: sales.length,
        sales_last_at: lastSaleAt,
        updated_at: new Date().toISOString(),
      });
      matchedLeads += 1;
...
async function syncTreatmentsOnce() {
  ...
  try {
    ...
    for (const tab of tabs) {
      const rows = await fetchRows(spreadsheetId, tab.title);
      const result = ingestSheetRows(tab.title, rows);
      imported += result.imported;
      withPhone += result.withPhone;
    }
    const attribution = applyAttribution();
    console.log(
      `[treatments-sync] OK: ${tabs.length} pestaña(s), ${imported} fila(s), ${withPhone} con telefono, ` +
      `${attribution.matchedLeads} lead(s) atribuido(s), ${attribution.attributedRevenue.toFixed(2)} EUR.`
    );
...
function startTreatmentSync() {
  ...
  if (!configured()) {
    console.warn(
      "[treatments-sync] Preparado pero pausado: faltan GOOGLE_SERVICE_ACCOUNT_EMAIL y/o GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY."
    );
    return null;
  }

  syncTreatmentsOnce().catch((err) => {
    console.error("[treatments-sync] Sincronizacion inicial fallida:", err.response?.data?.error?.message || err.message);
  });
  const timer = setInterval(() => {
    syncTreatmentsOnce().catch((err) => {
      console.error("[treatments-sync] Error:", err.response?.data?.error?.message || err.message);
    });
  }, SYNC_INTERVAL_MS);
